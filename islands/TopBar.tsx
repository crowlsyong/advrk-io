...
export default function TopBar() {
  ...
  const getLinkClass = (path: string) =>
    `flex items-center px-3 py-2 rounded-md text-sm font-medium ${
      activePath === path ? "bg-gray-200" : "text-gray-700 hover:bg-gray-100"
    }`;

  return (
    <nav class="bg-white border-b border-gray-300 shadow-sm fixed top-0 w-full z-50" ref={menuRef}>
      <div class="max-w-7xl mx-auto px-2 sm:px-6 lg:px-8">
        <div class="relative flex items-center justify-between h-16">
          <div class="absolute inset-y-0 left-0 flex items-center">
            ...
            <a href="/" class="ml-1 text-xl font-semibold">🧗 advrk.io</a>
          </div>
        </div>
      </div>

      <div
        class={`fixed inset-0 bg-white z-50 transition-transform transform top-0 left-0 bottom-0 w-64 bg-white shadow-lg space-y-1 p-4 ${
